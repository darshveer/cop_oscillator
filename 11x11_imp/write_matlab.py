...
def generate_node_names_block(rows: int, cols: int, layer: int = 1) -> str:
    """
    Generate MATLAB nodeNames cell array block like:
    ...
    lines = []
    for r in range(rows):
        row_names = [f"\"N_{r}_{c}_{layer}\"" for c in range(cols)]
        # Join with commas like MATLAB
        line = ",".join(row_names) + ", ..."
        lines.append(line)

    # Build full MATLAB block
    mat_lines = []
    mat_lines.append("nodeNames = { ...")
    for line in lines:
        mat_lines.append(line)
    mat_lines.append("};")

    # Indent for neatness (optional)
    block = "\n".join(mat_lines)
    block = indent(block, "")
    return block

def generate_matlab_code(rows: int, cols: int) -> str:
    node_block = generate_node_names_block(rows, cols)
...
"""
    ...
    return template.replace("<<NODE_NAMES_BLOCK>>", node_block)


def main():
    parser = argparse.ArgumentParser(
        description="Generate MATLAB script for node phase analysis with auto nodeNames."
    )
    parser.add_argument("--rows", "-r", type=int, required=True,
                        help="Number of node rows (e.g. 4).")
    parser.add_argument("--cols", "-c", type=int, required=True,
                        help="Number of node columns (e.g. 4).")
    parser.add_argument("--outfile", "-o", type=str, default=None,
                        help="Optional output .m file path. If omitted, prints to stdout.")
    args = parser.parse_args()

    code = generate_matlab_code(args.rows, args.cols)

    if args.outfile:
        with open(args.outfile, "w", encoding="utf-8") as f:
            f.write(code)
        print(f"MATLAB code written to {args.outfile}")
    else:
        print(code)
